...
/**
 * Remark plugin: converts [DOC-XXX] citation patterns in text into
 * inlineCode nodes so they can be styled as badges by the code component.
 */
        ...
        if (child.type === "text") {
          ...
          if (regex.test(child.value)) {
            ...
            while ((m = regex.exec(child.value)) !== null) {
              ...
              next.push({ type: "inlineCode", value: `DOC-${m[1]}` });
...
const defaultComponents = memoizeMarkdownComponents({
  ...
  code: function Code({ className, children, ...props }) {
    ...
    // Render [DOC-XXX] citations as colored pill badges
    ...
    if (
      ...
      return (
        <span className="inline-flex items-center rounded-full bg-sky-500/20 text-sky-300 px-1.5 py-0.5 text-[0.75em] font-semibold tracking-wide align-baseline mx-0.5">
          {text}
        </span>
      );
    }
